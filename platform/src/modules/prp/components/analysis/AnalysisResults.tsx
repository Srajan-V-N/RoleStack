...
import { Card } from '@/modules/prp/components/ui';
import type { AnalysisEntry, ExtractedSkills } from '@/modules/prp/lib/types';
          ...
          <div className="relative ml-4 border-l-2 border-accent pl-6">
            {entry.roundMapping.map((r, i) => (
              <div key={i} className="relative mb-8 last:mb-0">
                <div className="absolute -left-[calc(1.5rem+5px)] flex h-6 w-6 items-center justify-center rounded-full bg-accent text-xs font-bold text-white">
                  {i + 1}
                </div>
                <h4 className="text-sm font-bold text-gray-900">{r.roundTitle}</h4>
                <span className="text-xs font-medium text-accent">{r.focusAreas.join(', ')}</span>
      ...
      <div className="flex flex-wrap gap-2">
        {exportButtons.map(({ key, label, action }) => (
          <button
            key={key}
            onClick={action}
            className="rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-sm transition-colors hover:bg-gray-50 hover:text-accent"
          >
